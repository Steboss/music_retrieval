...
double mel_to_hz(double curr_mel){
  ...
  if (curr_mel >=min_log_mel){
    double freqs = min_log_hz * exp(logstep*(curr_mel-min_log_mel));
  }
  else{
    double freqs = f_min + (f_sp*curr_mel);
  }
...
double hz_to_mel(double frequency){
  ...
  if (frequency>= min_log_hz){
    double mels = min_log_mel + log(frequency/min_log_hz)/logstep;
  }
  else{
    double mels = (frequency - f_min)/f_sp ;
  }
...
double** mel(int sample_freq,int windowSize, double** weights ){
  ...
  mel_frequencies(melfreqs_size, fqmin, fqmax, melfreqs);  //nmels_+2 vector
  ...
  */
  ...
  ramps = (double**) malloc(n_mels+2*sizeof(double*));
             ...
             double *magnitude, int sample_freq, int length, int n_mfcc, double *cython_basis)
{
    ...
    int cols = ( (length/(windowSize/2)) -1);
    int rows = 1+floor(windowSize/2);
    ...
    //2. mel filter
    int n_mels = 128; //TODO this is hard coded
    int n_cols = 1+floor(windowSize/2);
    double **weights ;
    ...
    printf("Dot product n_mels * stft\n");
    ...
    int basis_cols =  ( (length/(windowSize/2)) -1);
    int basis_rows = n_mels;
    ...
    for (int i=0; i< n_mels; i++){
      for (int j=0; j< cols; j++){
        for (int k=0;k< rows; k++){
          //here we are converting the product from amplitude squared, as per a
          //spectrogram, to dB units
          curr_spectro = weights[i][j]*magnitude_array[k][j];
          //convert to dB
          if (curr_spectro>amin){
            log_spectro = 10.0*log10(curr_spectro);
